...
public class PageRankNode implements Writable {
    ...
    public float getPageRank(int i) {
        return pageranks.get(i);
    }

    public void setPageRank(int i, float value) {
        pageranks.set(i, value);
    }
    ...
    public void readFields(DataInput in) throws IOException {
        ...
        if(type.equals(Type.Structure) || type.equals(Type.Complete)){
            // structure only
            adjacenyList = new ArrayListOfIntsWritable();
            adjacenyList.readFields(in);
        }

        if(type.equals(Type.Complete) || type.equals(Type.Mass)){
            pageranks = new ArrayListOfFloatsWritable();
    ...
    public String toString() {
        return String.format("{%d %s %s}",
                nodeid,
                (pageranks == null ? "[]" : pageranks.toString()),
